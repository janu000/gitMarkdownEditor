...
- **Pipe Structural Highlighting:** Implemented a custom CodeMirror `ViewPlugin` (`pipeHighlightPlugin`) that dynamically highlights all pipe characters (`|`) whenever consecutive lines share the same number of pipes (n > 0). The highlighting color is theme-aware: `#f9e616` (yellow) in dark mode for high contrast, and `#f97316` (orange) in light mode. This aids in visually identifying table-like structures in Markdown before they are fully rendered.
